...
//        System.out.println("------ Print Infinite Stream ---------- ");
//        IntStream.iterate(0,i->i+1).forEach(System.out::println);

        int N = 1025;
        System.out.println("List with Element " + N + ":" + createList(N));

        System.out.println("-------iterate over a stream with indices in Java 8");
        String[] strArray = {"P", "o", "k", "e", "m", "o", "n"};
        IntStream.range(0, strArray.length).mapToObj(index -> String.format("%d -> %s", index, strArray[index]))
                .forEach(System.out::println);

        System.out.println("-------IntStream to String in Java 8");
        IntStream intStream1 = "Pokemons".chars();
        String string = intStream1.collect(StringBuilder::new,StringBuilder::appendCodePoint,StringBuilder::append)
                .toString();
        System.out.println("String: " +string);


        System.out.println("---------Collectors to Map Method-----");
        Stream<String[]> strStream = Stream.of(new String[][] {
                {"A","Pokemon"},
                {"B", "Johto"},
                {"C", "Kanto"}
        });
        Map<String,String> map = strStream.collect(Collectors.toMap(p->p[0],p->p[1]));
        System.out.println("Map: "+map);

        Stream<String[]> strStreams = Stream.of(new String[][] {
                {"A","Pokemon"},
                {"B", "Johto"},
                {"C", "Kanto"},
                {"D","Sinnoh"},
        ...
        Map<String,String> map2 = strStreams.collect(Collectors.toMap(p->p[0],p->p[1],(s,a)->s +", "+a));
        System.out.println("Map: "+map2);


        Stream<String[]> strStreams1 = Stream.of(new String[][] {
                {"A","Pokemon"},
                {"B", "Johto"},
                {"C", "Kanto"},
                {"D","Sinnoh"},
        ...
        LinkedHashMap<String,String> map3 = strStreams1.collect(Collectors.toMap(p->p[0],p->p[1],(s,a)->s +", "+a,LinkedHashMap::new));
        System.out.println("Map: "+map3);

        System.out.println("---------Collectors to Set Method-----");
        Stream<String> strSet = Stream.of("Pokemon","Johto","Kanto", "Sinnoh",
                "Unova", "Unova","Johto","Kanto");
